...
|---|---|---|---|---|---|---|
| Rotation de la mini-carte façon « suivant le joueur » (option, nord fixe par défaut) | — | — | — | ✅ Fait v0.21 | — | Réglage « Mini-carte suit le cap » dans Options (Affichage), off par défaut. Repère « N » affiché seulement en mode suivi (inutile en nord fixe, où haut = nord par convention). |
| Boussole/indicateur de cap en haut du HUD | — | — | — | ✅ Fait v0.21 | — | Lecture cardinal + degrés (« NE 047° ») sous le nom de quartier, mise à jour en direct. Version simplifiée d'une bande graphique complète (jugée non nécessaire vu ce que la mini-carte donne déjà). |
...
| Distance chiffrée jusqu'à l'objectif | — | — | — | ✅ Fait v0.20 | — | Affichée sous le texte d'objectif (« 240 m » ou « 1.2 km »), mise à jour en direct. |
| Nom de quartier affiché en continu en jeu | — | — | — | Déjà fait (erreur d'audit) | — | Correction : `#district` affichait déjà le quartier en jeu en permanence avant cet audit (`hud.js` `update()`), l'affirmation du 23/09 était fausse. Un vrai nom de **rue** (pas seulement de quartier) demanderait un système de tronçons nommés qui n'existe pas — non proposé, coût largement supérieur au bénéfice pour une ville de 6 quartiers. |
...
| Raccourci pour masquer/afficher la mini-carte | — | — | — | Déjà couvert (erreur d'audit) | — | Correction : le réglage « Mini-carte » (Options → Affichage) fait déjà ça, persisté, depuis avant cet audit. Un raccourci clavier direct resterait un vrai P3 si le détour par le menu pause gêne en jeu, mais rien ne l'indique pour l'instant. |
...
| Barre de progression pour le rechargement | — | — | — | ✅ Fait v0.20 | — | Barre sous les munitions, visible seulement pendant le rechargement, largeur proportionnelle au temps restant. |
