...
| Personnage collé à un mur pendant le tutoriel | — | — | — | ✅ Fait v0.26 | — | Repéré en jouant (capture d'écran, 24/09) : le marqueur de l'étape 1 et le point d'apparition de la voiture (`story.js`, mission `intro`) étaient des décalages fixes sans vérifier la présence d'un bâtiment. Remplacés par un point de rue (`roadPointAround`) et la place de stationnement la plus proche (`nearestParkedSpot`), comme les autres missions. |
